...
void GameLogic::RecordMove(int x, int y, int Piece)
{
  turnNum++;
  //add move to db
 cob_init(0, NULL);
 addtofile(&turnNum, &x, &y, &Piece, &CurrentTurn);
}
 ...
 */
 ...
 if(recordGame)
 {
   RecordMove(CPUmove.x, CPUmove.y, CPUmove.piece);
 }
...
Fl_Toggle_Button* GameLogic::FindButton(Fl_Double_Window* win, int x, int y)
{
  ...
  for(int i = 0; i < win->children(); i ++)
  {
    ...
    if(Fl_Toggle_Button *button = static_cast<Fl_Toggle_Button *> (widget))
    {
